...
const eleBox = function (electron) {
    if(!electron){
        return;
    }
...
eleBox.prototype.ready = function (url, opt, debug = false) {
    ...
    const createWindow = (url = "https://github.com", opt, debug = false) => {
        const defaultOpt = {
        ...
        if (defaultOpt.ipcMode) {
            handleIpcThings(defaultOpt.ipcListener, defaultOpt.ipcSync, debug,ipcMain);
        }
